...
class MemoryWriterPrivate(nn.Module):
    """Differentiable memory write without incoming communication.

    Given the agent's observation encoding and memory summary, computes
    soft write addresses, a write gate, and candidate content, then
    updates each memory cell with a gated interpolation.
    """
    ...
    def __init__(
        ...
        # Input dimension: [u; z]
        context_dim = hidden_dim + mem_dim
        ...
        # Candidate content: MLP from [u]
        candidate_in = hidden_dim
        self.candidate_net = nn.Sequential(
            nn.Linear(candidate_in, mem_dim),
            nn.ReLU(),
            nn.Linear(mem_dim, mem_dim),
    ...
    def forward(
        ...
        Returns:
            new_memory: (N, K, mem_dim) — updated memory cells
        """
        context = torch.cat([u, z], dim=-1)

        qg = self.write_query_gate(context)
        wq = qg[:, :self.mem_dim]
        w = torch.sigmoid(qg[:, -1:])

        wk = self.write_key(memory)
        beta = torch.einsum("nd,nkd->nk", wq, wk)
        beta = F.softmax(beta / self.write_scale, dim=-1)

        candidate_input = u
        m_tilde = self.candidate_net(candidate_input)
